...
async function connectToAgent(agentId) {
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.on('open', () => {
      ...
      // Dynamic configuration based on env
...
async function endConversation(conversationId) {
  ...
  // If this is a local-only ID (generated by us), we can't call the API with it.
  // The session ends when the WebSocket disconnects.
  if (conversationId && conversationId.startsWith('conv_')) {
    logger.info('Skipping API delete for local conversation ID (WebSocket close is sufficient)', { conversationId });
    return;
  }
